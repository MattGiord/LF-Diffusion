...
%   1.2 Gradient descent initialisation
%
% 2. Bayesian nonparametric inference from low frequency diffusion data with 
%    truncated Gaussian series priors via the gradient descent algorithm for 
%    MAP computation
%
...
stepsize=.00001;
MAP_gd_length=500;
MAP_gd_theta=zeros(J_basis,MAP_gd_length);
    % vector to store the Fourier coefficients along the gradient descent
MAP_gd_theta(:,1)=theta_init;
    % initialation point
MAP_gd_pdf=zeros(1,MAP_gd_length-1);
    % vector to store the posterior densities along the gradient descent
MAP_gd_loglik=zeros(1,MAP_gd_length-1);
    % vector to store the loglikelihood along the gradient descent
inv_prior_cov=diag(lambdas_basis.^(2*prior_regularity))/prior_variance;
    % inverse prior covariance matrix
index_conv=MAP_gd_length;
    % index for convergence criterion
tic 

for grad_desc_step=1:(MAP_gd_length-1)
    %disp(['Gradient descent step n. ', num2str(grad_desc_step)])
    
    F_current_mesh_prior=(e_basis*MAP_gd_theta(:,grad_desc_step))';
    ...
    % compute posterior density of current gradient descent step
    MAP_gd_loglik(grad_desc_step) = sum(log(abs(trds)));
    MAP_gd_pdf(grad_desc_step)=MAP_gd_loglik(grad_desc_step) ...
        -.5*MAP_gd_theta(:,grad_desc_step)'*inv_prior_cov ...
        *MAP_gd_theta(:,grad_desc_step);
    ...
    grad=sum(Frech_derivs./trds)' - inv_prior_cov*MAP_gd_theta(:,grad_desc_step);
    
    % Gradient descent update
    MAP_gd_theta(:,grad_desc_step+1) = MAP_gd_theta(:,grad_desc_step)...
        +stepsize*grad;

    % Check convergence criterion
    if norm(MAP_gd_theta(:,grad_desc_step+1)- MAP_gd_theta(:,grad_desc_step))<.001
        index_conv=grad_desc_step+1;
        disp(['Convergence reached at step n. ', num2str(index_conv)])
        break
    end
...
%%
% Plot the posterior density along the gradient descent

index_conv=min(MAP_gd_length,index_conv);
...
plot(MAP_gd_pdf(1:index_conv-1),'LineWidth',5)
yline(post_pdf0,'r','LineWidth',2)
legend('log\pi(\vartheta_m|X^{(n)})','FontSize',20)
xlabel('Gradient descent step','FontSize',20)

%%
% Plot the norm difference between subsequent gradient descent steps

theta_diff = MAP_gd_theta(:,2:index_conv) ...
    -MAP_gd_theta(:,1:(index_conv)-1);
theta_diff_norm=vecnorm(theta_diff);

figure()
axes('FontSize', 20, 'NextPlot','add')
plot(theta_diff_norm,'LineWidth',5)
yline(0.001,'r','LineWidth',2)
xlabel('Gradient descent step','FontSize',20)
legend('|\vartheta_{m+1} - \vartheta_m|','.001','FontSize',20)

%%
% MAP estimator

% Compute MAP estimate
theta_MAP = MAP_gd_theta(:,index_conv);
F_MAP_mesh_prior=zeros(1,mesh_nodes_num_prior);
for j=1:J_basis
    F_MAP_mesh_prior = F_MAP_mesh_prior+theta_MAP(j)*e_basis(:,j)';
end

% Plot MAP and estimation erorr
figure()
...
figure()
pdeplot(model_prior,'XYData',F_MAP_mesh_prior,'ColorMap',jet)
%title('MAP estimate (via gradient descent)','FontSize',20)
clim([min(F0_mesh_prior),max(F0_mesh_prior)])
axis equal
colorbar('Fontsize',20)
xticks([-.5,-.25,0,.25,.5])
...
pdeplot(model_prior,'XYData',F0_mesh_prior-F_MAP_mesh_prior,'ColorMap',hot)
...
MAP_estim_error = norm(theta_MAP-F0_coeff);
disp(['L^2 estim error = ', num2str(MAP_estim_error)])
